...
%% First spiral
r0 = 0.5*D/sin(alpha_conv+pi/2);
theta = alpha_div-pi/2;
theta0 = theta;

while theta > -3*pi/2;
    
    angle = cot(alpha_div) .* (theta0 - theta);
    r = r0*exp(angle);
...
while theta < alpha_conv - 2*pi 
    
    angle1 = cot(alpha_div) .* (theta01 - theta);
    r1 = r01*exp(angle1);
...
while theta > alpha_div+pi
    
    angle4 = cot(alpha_conv) .* (theta04 - theta);
    r4 = r04*exp(angle4);
